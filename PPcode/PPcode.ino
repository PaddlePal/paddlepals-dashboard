...
void doReset() {
  ...
  hitDataString = "";
  imuDataString = "";
  // Send status directly before stopping the ticker —
  // bleTicker.stop() would strand the message if we used statusString instead
  if (BLE.connected()) {
    fsrCharacteristic.writeValue("STATUS:SESSION_RESET");
  }
  statusString = "";
